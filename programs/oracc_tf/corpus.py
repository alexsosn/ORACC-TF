...
class CorpusBuildReport:
    ...
    translation_source_supplied: bool = False
    translation_units: int = 0
    translated_documents: int = 0
    translation_missing_documents: int = 0
    translation_notes: int = 0
    ...
    def report(self) -> str:
        ...
        if self.translation_source_supplied:
            rows.extend((
                f"translation units          : {self.translation_units:>8,}",
                f"translation notes          : {self.translation_notes:>8,}",
                f"translated documents       : {self.translated_documents:>8,}",
                f"translation missing docs   : {self.translation_missing_documents:>8,}",
            ))
        return "\n".join(rows)
...
def _normalize_tf_generated_metadata(out_dir: Path) -> None:
    """Remove Text-Fabric's volatile writer timestamp from publishable bytes.

    ``Fabric.save()`` injects ``@dateWritten=<wall clock>`` into every feature
    file. That metadata is generated by Text-Fabric rather than derived from
    ORACC source state, and it makes otherwise identical rebuilds hash
    differently. P-005 provenance lives in the distribution manifest, so the
    generated timestamp is removed instead of replaced by a fabricated time.
    Only the metadata header before the first blank line is touched.
    """
    for path in sorted(out_dir.glob("*.tf")):
        if not path.is_file():
            continue
        temp = path.with_name(f".{path.name}.normalize")
        try:
            payload = path.read_bytes()
            header, separator, body = payload.partition(b"\n\n")
            if not separator:
                raise CorpusBuildError(
                    f"Text-Fabric feature lacks a metadata/body separator: {path}"
                )
            lines = header.split(b"\n")
            normalized = b"\n".join(
                line for line in lines if not line.startswith(b"@dateWritten=")
            )
            if normalized != header:
                temp.write_bytes(normalized + separator + body)
                temp.replace(path)
        except OSError as exc:
            raise CorpusBuildError(
                f"cannot normalize Text-Fabric generated metadata in {path}: {exc}"
            ) from exc
        finally:
...
def build_tf(
    ...
    *,
    ...
    translation_source_supplied = translations_by_document is not None
    translation_index = {
        key: tuple(units)
    ...
    for edition in editions:
        ...
        seen_document_keys.add(edition.key)
        ...
        if translation_units:
            ...
            for unit in translation_units:
                ...
                unit_slots = {
                    ...
                    for slot in slot_plan.section_slots[line]
                }
                if not unit_slots:
                    raise CorpusBuildError(
                ...
                graph.feature(
                    ...
                    translation_source_sha256=unit.source_sha256,
                    translation_source_url=unit.source_url,
                    translation_license=unit.source_license,
                    translation_license_url=unit.source_license_url,
                )
                graph.edge("translation_document", node, document_node)

                for note in unit.notes:
                    note_key = (edition.key, note.source_id)
                    note_node = translation_note_nodes.get(note_key)
                    if note_node is None:
                        ...
                        graph.feature(
                            ...
                            document_key=edition.key,
                            translation_note_text=note.text,
                            translation_note_text_raw=note.text_raw,
                            translation_source=note.source_name,
                            translation_source_sha256=note.source_sha256,
                            translation_source_url=note.source_url,
                            translation_license=note.source_license,
                            translation_license_url=note.source_license_url,
                        )
    ...
    unknown_translation_documents = set(translation_index) - seen_document_keys
    if unknown_translation_documents:
        sample = sorted(unknown_translation_documents)[:5]
        raise CorpusBuildError(
            f"translation index contains documents absent from build: {sample!r}"
        )
    ...
    _normalize_tf_generated_metadata(out_dir)

    missing_translation_documents = (
        document_count - translated_document_count
        if translation_source_supplied
        else 0
    )
    return CorpusBuildReport(
        ...
        translation_source_supplied=translation_source_supplied,
        translation_units=translation_unit_count,
        translated_documents=translated_document_count,
        translation_missing_documents=missing_translation_documents,
        translation_notes=len(translation_note_nodes),
    )
