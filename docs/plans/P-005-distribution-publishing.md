...
- TF schema version is SemVer and is not source-state identity.
...
`manifest.json` is deterministic and records dataset id, TF version, ORACC-TF builder commit, source-state identity when available, tree/artifact integrity information, and the exact relative TF root. A provenance field that cannot yet be established must be explicitly unavailable/blocking; it must not be fabricated.
...
- same immutable identity + same bytes: idempotent no-op;
- same immutable identity + different bytes: hard conflict;
...
- corrections create a new version/source-state release; published immutable identities are not rewritten.
...
4. same identity/same bytes is idempotent;
5. same identity/different bytes fails closed;
6. staged output contains no unrelated raw/build/research paths;
7. manifest binds distribution -> ORACC-TF builder commit -> source-state field explicitly;
8. incomplete TF warp/sidecar publication is rejected before visibility;
9. unsafe/colliding repository-name derivations fail or disambiguate deterministically;
10. a representative generated repository can be acquired at an immutable revision and its TF root loaded;
11. benchmark accounting separates metadata bytes from materialized TF bytes.
