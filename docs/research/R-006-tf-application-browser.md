...
The comparison was reproduced against executable current surfaces rather than copied from BHSA prose:

- ORACC-TF research base `cc078176195071af1e9ef907decafa65066ed91a`, whose package constraint is `text-fabric>=13.1,<14`;
...
Text-Fabric 13.1 supplies the browser/server and advanced-app runtime. A corpus app configures that runtime; it does not need its own Flask/server implementation. `config.yaml` and `app.py` are both optional. `data:/path/to/data` works with the advanced-app loader, while repository-style loading discovers a conventional app next to TF data. Text-Fabric also loads app CSS/static assets and supports optional Python specialization where declarative configuration is insufficient.

BHSA confirms the intended pattern. Its current app consists primarily of `app/config.yaml`, one static logo, and a very small `app.py` whose only corpus-specific method is `getLexId`. Its text/lexeme formats are declared in `tf/2021/otext.tf`; app config mainly controls display, feature preloading, missing-value presentation, documentation, provenance/web links, and writing-system policy.
...
The research harness `scripts/research_issue69_tf_app.py` loads the real built `assyrian-royal-inscriptions` corpus through Text-Fabric's actual advanced-app API, not through a mock. The initial pinned measurement artifact from research head `e0ad631e8ff683b63a842c3a920c1976981c4fdc` is GitHub Actions artifact `10033165907` with artifact digest `sha256:233c72bb51c445150799f3df1e30d03e20722770accb61d4292e1c1f0a34571c`. Later issue-69 hardening pinned the dedicated workflow to Text-Fabric 13.1.0 and preserved the same research conclusions.
...
To characterize the browser stack without disguising that limitation, the research harness starts from a successfully loaded real `AdvancedApp`, passes it through `makeTfKernel()`, and uses Text-Fabric's browser Flask factory. This is evidence about rendering/query/export route behavior **inside the Text-Fabric browser stack**, not proof that a clean published repository will be discovered correctly by the public browser command.

On the vanilla fixture, `/passage` and `/query` return 200 while `/` and `/export` fail during corpus-specific header rendering. That is useful baseline behavior, not the desired product contract. Production repository-style app discovery and all-route browser acceptance remain mandatory in #71/#75.
...
| Declarative app config | TF 13.1 + current BHSA | Generate ordinary declarative config by default |
...
| `typeDisplay` | TF 13.1 configuration surface + BHSA precedent | Generate reviewed per-type policy from actual emitted schema |
| Static/CSS | TF app discovery/static support | Allowed for readability/branding, no semantics; own it explicitly |
...
| Dataset app authoring | User preference + derivable ORACC-TF state | Generated by default; manual source only as narrow reviewed override |
...
## Generated-by-default app artifacts

The normal ORACC-TF workflow must **not** require maintainers to create a full app directory or copy/edit another corpus's `config.yaml` whenever a new semantic dataset is published. If the value is derivable, generation is the source of truth.

The intended flow is:

```text
dataset registry
+ actual emitted TF schema/features/node types
+ reviewed cross-dataset presentation policy from R-006/#70
+ release/distribution manifest + upstream lock
+ generated documentation registry/links
+ OPTIONAL narrow dataset override
    -> deterministic generated otext/app config/static manifest inputs
    -> manifest-owned published app/
```

The published `app/` tree is therefore a **build artifact**. The central repository should own generic generator logic and reviewed defaults, not one manually synchronized full app tree per dataset. A dataset-specific override is legitimate only when behavior is genuinely non-derivable (for example exceptional branding/CSS or a verified source-link resolver that cannot be represented declaratively). Overrides must be schema-validated, narrow, independently reviewed, and unable to replace derivable release/schema/provenance facts.

In particular, release/version/licence/source state, emitted feature lists, section/node inventory, documentation destinations, default generated formats adopted by the shared corpus model, and provenance bindings are not manual override material. Adding a normal new dataset should succeed without creating `apps/<dataset>/config.yaml` by hand.

This is both an ergonomics and correctness requirement: copy-and-edit corpus apps would create drift, duplicate policy and make cross-corpus rollout of schema/display fixes unreliable.

## Provenance and source links

Release facts must flow from authoritative build inputs and into the same generated app artifact:
...
Do not maintain a second manually synchronized corpus version, source hash, licence, DOI, or release state in `config.yaml` or in an override.
...
The central builder should therefore prefer generic generator source plus optional narrow overrides rather than full hand-maintained apps, e.g. conceptually:

```text
programs/oracc_tf/appgen.py
app-policy/defaults...
app-overrides/<dataset>/...   # optional and exceptional
```
...
- **One global manually conditional app for every future ORACC dataset:** couples unrelated presentation and encourages dataset-condition spaghetti.
- **One hand-created full app tree per dataset:** duplicates derivable policy, creates drift, and makes new-dataset onboarding unnecessarily manual.
...
- #70: synthesize this research into the next P-series application plan, with generated-by-default app creation as a cross-cutting invariant;
- #79: extend P-005 path/manifest ownership for canonical app/docs/release support paths;
- #71 / Phase A: generic app generator, narrow override contract, materialization and Text-Fabric discovery, coordinated with #79;
- #72 / Phase B: production cuneiform, word-level transliteration and lexeme formats, emitted generically where derivable;
- #73 / Phase C: reviewed `typeDisplay`, measured preload policy, docs links and CSS/static policy, with ordinary `config.yaml` generated rather than hand-authored;
...
- an ordinary new dataset requires copying/hand-editing a full existing app rather than generic generation plus a narrow explicit override;
