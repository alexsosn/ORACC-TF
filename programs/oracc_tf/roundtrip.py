...
def evaluate_word(word: words.WordRecord) -> WordRoundTrip:
    ...
    # otherwise plain sign spellings: GDL uses ḫ while the lemmatiser's written
    # ``form`` uses h. Apply this only after structural/markup explanations so
    # it does not steal their primary exception reason.
    elif "ḫ" in candidate and candidate.replace("ḫ", "h") == word.form:
